...
func (a cloudRunAdapter) ApplyImage(ctx context.Context, env *manifest.Environment, svc *manifest.Service, timeout time.Duration) (*cloudrun.Deployed, error) {
	return a.c.ApplyImage(ctx, env, svc, timeout)
}
